...
def main(args):
    ...
    for i in range(num):
        print("[",str(i+1),'/',num,"]")
        temp=task.evaluate(model, res, graph)  # evaluate
        resultMap=getSum(resultMap,temp)
    resultMap=getAverage(resultMap,num)
    print('The final result is ')
    print(resultMap)
    endtime = datetime.datetime.now()
    print('Finished evaluation. Time used = ',(endtime - starttime).seconds,'s')
